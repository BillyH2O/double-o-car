...
export default function NotFound() {
  ...
  // Extraire la locale depuis l'URL pour traduire le contenu
  const segments = pathname ? pathname.split('/').filter(Boolean) : [];
  const possibleLocales = ['fr', 'en', 'nl'];
  
  // Si le premier segment est une locale valide, on l'utilise
  const locale = (segments.length > 0 && possibleLocales.includes(segments[0])) 
